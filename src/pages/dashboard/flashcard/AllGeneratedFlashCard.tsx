...
import { useGetAllFlashCardQuery } from "@/store/features/flashCard/flashCard.api";

export default function AllGeneratedFlashCard() {
  const { data } = useGetAllFlashCardQuery(undefined);
  console.log(data);
  ...
  return (
    <div>
      <div className="flex items-center gap-3">
        <Link to={"/dashboard/flashcard-collection"} className="mb-7">
          <ArrowLeft />
        </Link>
        <DashboardHeading
      ...
      <div className="bg-white border border-slate-300 rounded-[8px] py-5 pl-7 pr-5">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            {data?.data?.map((_: any, idx: number) => (
              <FlashCard key={idx} />
            ))}
          </div>
        </div>
