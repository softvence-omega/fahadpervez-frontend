// import { Question } from "../types/case";

import { Question } from "./type/case";

type Props = {
  question: Question;
  selectedOptionId: string;
};

const EvidenceReview: React.FC<Props> = ({ question, selectedOptionId }) => {
  return (
    <div className="p-6 bg-white rounded-xl shadow">
      <h3 className="text-lg font-semibold mb-4">
        Your Differential Diagnosis Table
      </h3>
      <div className="space-y-4">
        {question.options.map((opt) => (
          <div
            key={opt.id}
            className={`p-4 border rounded-lg ${
              selectedOptionId === opt.id ? "border-blue-600" : "border-gray-300"
            }`}
          >
            <div className="flex justify-between items-center mb-2">
              <span className="font-medium">{opt.text}</span>
              {selectedOptionId === opt.id && (
                <span className="text-xs px-2 py-1 bg-blue-100 text-blue-800 rounded">
                  Your Choice
                </span>
              )}
            </div>
            <p className="text-green-600 font-medium">Supporting Evidence:</p>
            <ul className="list-disc list-inside text-sm text-gray-700">
              {opt.supportingEvidence.map((e, i) => (
                <li key={i}>{e}</li>
              ))}
            </ul>
            <p className="text-red-600 font-medium mt-2">Refuting Evidence:</p>
            <ul className="list-disc list-inside text-sm text-gray-700">
              {opt.refutingEvidence.map((e, i) => (
                <li key={i}>{e}</li>
              ))}
            </ul>
          </div>
        ))}
      </div>
    </div>
  );
};

export default EvidenceReview;
