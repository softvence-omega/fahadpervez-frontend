...
import { z } from "zod";
import { useGetAllSocialPostQuery } from "@/store/features/socialPost/social.api";
...
//   useEffect(() => {
//     if (data?.data) {
//       const mappedPosts: Post[] = data.data.map((item: any) => ({
//         id: item._id,
//         author: {
//           name:
...
//               ? `${item.postedBy.firstName} ${item.postedBy.lastName}`
//               : "Unknown",
//           handle: `@${item.postedBy?._id ?? "unknown"}`,
//           avatar: item.postedBy?.profile_photo ?? img,
//           timeAgo: new Date(item.createdAt).toLocaleString(),
//         },
//         content: item.content,
//         likes: item.reaction?.length ?? 0,
//         comments: item.comments?.length ?? 0,
//         shares: item.share ?? 0,
//         badge: item.profileType === "admin_profile" ? "Admin" : undefined,
//       }));
...
//       const submissionFormData = new FormData();
//       submissionFormData.append("content", validatedData.content);
//       if (validatedData.image) submissionFormData.append("image", validatedData.image);
...
//           <div className="space-y-4">
//             {posts?.map((post) => (
//               <PostCard key={post.id} post={data?.data} onLike={handleLike} />
//             ))}
//           </div>
