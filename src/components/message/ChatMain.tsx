...
import { useGetGroupMessagesQuery, useSendGroupMessageMutation } from "@/store/features/group/groupApi";
...
const ChatMain: React.FC<ChatMainProps> = ({
  ...
  const getHeaderTitle = () => {
    ...
    if (activeUser.userType === "group") return activeUser.name || activeUser.groupName;
  ...
  const getHeaderSubtitle = () => {
    ...
    if (activeUser.userType === "group")
      return `${activeUser.memberCount || activeUser.groupMembers?.length || 0} members`;
      ...
      <div className="bg-white p-4 flex items-center flex-shrink-0 rounded-t-lg overflow-hidden border-b border-gray-200">
        ...
        <div className="flex items-center flex-1">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-300 rounded-full flex items-center justify-center mr-3 overflow-hidden">
             {activeUser?.userType === "group" && activeUser.groupLogo ? (
                <img src={activeUser.groupLogo} alt="" className="w-full h-full object-cover" />
             ) : activeUser?.userType === "group" ? (
        ...
        {displayMessages.length === 0 ? (
            ...
            [...displayMessages].reverse().map((message: any) => {
                ...
                const senderName = isGroupMsg 
                  ? (message.senderId?.profile_id?.firstName || message.senderId?.profile_id?.name || "Unknown") 
                  : message.userName;
                  
                const senderPhoto = isGroupMsg 
                  ? (message.senderId?.profile_id?.profilePhoto || message.senderId?.profile_id?.profileImg) 
                  : message.userAvatar; 
                ...
                return (
                  <div key={msgId} className="bg-white rounded-lg p-4 shadow-sm">
                    ...
                    {fileUrl && (
                        <div className="mb-2">
                            {/* Check if image */}
                            {fileName?.match(/\.(jpeg|jpg|png|gif)$/i) || fileUrl.match(/\.(jpeg|jpg|png|gif)/i) ? (
                                <img src={fileUrl} alt="attachment" className="max-w-xs rounded border" />
                            ) : (
                                <a href={fileUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-blue-600 underline bg-blue-50 p-2 rounded max-w-xs">
